 ...
 */
 let banner3 = {
    ...
    rollInit: function (newinterval) {
        ...
        //현재 배너
        let firstitem = document.querySelector('.rollimgs3 li');
        if(firstitem){
            firstitem.classList.add('currentroll');
        }
        //다음 배너
        let seconditem = document.querySelectorAll('.rollimgs3 li')[1];
        if(seconditem){
            seconditem.classList.add('nextroll');
        }
        //이전 배너
        document.querySelector('.rollimgs3 li:last-child').classList.add('prevroll');
    ...
    //다음 이미지 있으면 다음 롤링 이미지로 선택, 없으면 첫번째 이미지를 롤링 이미지로 지정
        if(document.querySelector('.currentroll').nextElementSibling){
            ...
            document.querySelector('.rollimgs3 li').classList.add('nextroll');
        }
    }
}
document.addEventListener('DOMContentLoaded', function(){
    banner3.rollInit(4000); // 배너 롤링
});